...
class Article extends Init
{
	...
	function index(){
		...
		$articles = $this->model->get_list($map,$order,$page_size);
		return view('list',['category_id'=>$category_id,'articles'=>$articles,'url_params_arr'=>$url_params_arr,'url_params_str'=>$url_params_str]);
	}
	...
	function edit(){
		...
		$model_category_select_option = $this->category_model->get_model_category_select_with_option($article['category_id']);
		return view('edit',array('article'=>$article,'model_category_select_option'=>$model_category_select_option));
	}
