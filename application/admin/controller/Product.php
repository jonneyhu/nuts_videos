...
class Product extends Init
{
	...
	{
		parent::_initialize();
		$this->model = model('common/product');
	...
	function index(){
		...
		$products = $this->model->get_list($map,$order,$page_size);
		return view('list',['category_id'=>$category_id,'products'=>$products,'url_params_arr'=>$url_params_arr,'url_params_str'=>$url_params_str]);
	}
	...
	function edit(){
		...
		$product = $this->model->where('id',input('param.id'))->find();
		$model_category_select_option = $this->category_model->get_model_category_select_with_option($product['category_id']);
		return view('edit',array('product'=>$product,'model_category_select_option'=>$model_category_select_option));
	}
