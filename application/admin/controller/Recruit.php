...
* 招聘控制器
*/
class Recruit extends Init
{
	
	function _initialize()
	{
		parent::_initialize();
		$this->model = model('common/recruit');
	...
	function index(){
		...
		$recruits = $this->model->get_list($map,$order,$page_size);
		return view('list',['category_id'=>$category_id,'recruits'=>$recruits,'url_params_arr'=>$url_params_arr,'url_params_str'=>$url_params_str]);
	}
	...
	function edit(){
		...
		$recruit = $this->model->where('id',input('param.id'))->find();
		$model_category_select_option = $this->category_model->get_model_category_select_with_option($recruit['category_id']);
		return view('edit',array('recruit'=>$recruit,'model_category_select_option'=>$model_category_select_option));
	}
