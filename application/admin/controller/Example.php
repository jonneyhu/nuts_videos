...
class Example extends Init
{
	...
	function index(){
		...
		$examples = $this->model->get_list($map,$order,$page_size);
		return view('list',['category_id'=>$category_id,'examples'=>$examples,'url_params_arr'=>$url_params_arr,'url_params_str'=>$url_params_str]);
	}
	...
	function edit(){
		...
		$example = $this->model->where('id',input('param.id'))->find();
		$model_category_select_option = $this->category_model->get_model_category_select_with_option($example['category_id']);
		return view('edit',array('example'=>$example,'model_category_select_option'=>$model_category_select_option));
	}
