	...
	function edit(){
		...
		if(request()->isPost()){
			...
			if(empty($params['description']) && !empty($params['content'])){
				$params['description'] = mb_substr(strip_tags($params['content']), 0,200,'utf-8');
			}
			$result = $this->model->edit($params,$page?true:false);
			if($result){
				return json(array('code'=>200,'msg'=>'编辑成功','description'=>$result->description));
			}else{
				return json(array('code'=>0,'msg'=>'编辑失败'));
			}
		}
		return view('edit',['page'=>$page,'category_id'=>$category_id]);
	}
}