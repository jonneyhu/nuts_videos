...
<div class="layui-tab layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      ...
      <a href="<?php echo url('category/add') ?>"><li class="layui-this">添加栏目</li></a>
      ...
      <div class="layui-tab-item layui-show">
        <form class="layui-form">
          <div class="layui-tab layui-tab-card">
            ...
            <div class="layui-tab-content">
              <div class="layui-tab-item layui-show">
                <?php //echo Form::select('model_id','','选择模型','模型',$model_select,'required','model_id');?>
                  <input type="hidden" name="model_id" value="10">
                <?php echo Form::select('parent_id','','上级栏目','上级栏目',$category_select);?>
                <?php echo Form::input('name','','栏目名称','栏目名称','请输入栏目名称','required');?>
                <?php //echo Form::input('en_name','','英文名称','栏目名称','请输入英文名称');?>
                <?php echo Form::file('image_url','','栏目图片','栏目图片','图片地址','','选择','images');?> 
                <?php //echo Form::file('wap_image_url','','移动端栏目图片','移动端栏目图片','图片地址','','选择','images');?>
                ...
                <?php //echo Form::radio('is_cover',1,'是否显示封面','无封面页，则进入下级第一个栏目列表页',array(1=>'是',0=>'否'));?>
              </div>
              <div class="layui-tab-item">
                <?php echo Form::input('index_template','','封面页模版','单页模型只需要填写封面页模板','请输入封面页模版');?>
                <?php echo Form::input('list_template','','列表页模版','','请输入列表页模版');?>
                <?php echo Form::input('show_template','','详情页模版','','请输入详情页模版');?>
              </div>
              <div class="layui-tab-item">
                <?php echo Form::input('meta_keywords','','栏目页面关键词','关键字中间用半角逗号隔开','请输入栏目页面关键词');?>
                <?php echo Form::textarea('meta_description','','栏目页面描述','针对搜索引擎设置的网页描述','请输入栏目页面描述');?>
              </div>
