...
<div class="layui-tab layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      ...
      <a href="<?php echo url('download/index','category_id='.$category_id) ?>"><li>列表</li></a>
      <a href="<?php echo url('download/add','category_id='.$category_id) ?>"><li class="layui-this">添加</li></a>
    </ul> 
    <div class="layui-tab-content">
       <form class="layui-form">
        <div class="layui-tab-item layui-show">
          ...
          <?php echo Form::file('file_url','','下载文件','文件','文件地址','','选择','file');?>
          <?php echo Form::input('filename','','文件名','前台下载之后的文件名','请输入文件名');?>
          <?php echo Form::input('demo_url','','演示地址','链接以http://开始','请输入演示地址');?>
          <?php echo Form::$settings['editor']('content','','文件详情');?>
          ...
          <?php echo Form::input('download_num',0,'下载量','请输入数字','请输入下载量，默认是0','number');?>
          <?php echo Form::input('url','','链接地址','外链地址，非外链文件则留空','以http://开头');?>
  ...
  form.on('submit(download_add)', function(data){
    ...
    $.ajax({
      ...
      success:function(data){
        if(data.code == 200){
          show_msg(data.msg,1,'{:url("download/index",["category_id"=>$category_id])}');
        }else{
          show_msg(data.msg,2);
        }
      },
