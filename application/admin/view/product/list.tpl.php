...
<div class="layui-tab layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      ...
      <a href="<?php echo url('product/index','category_id='.$category_id) ?>"><li class="layui-this">列表</li></a>
      <a href="<?php echo url('product/add','category_id='.$category_id) ?>"><li>添加</li></a>
    </ul>
    <div class="layui-tab-content">
      <div class="layui-tab-item layui-show">
        <div class="table_top_bar"> 
          ...
          <div class="spacer-gray"></div>
          <a href="<?php echo url('product/index','category_id='.$category_id) ?>"><i class="layui-icon">&#x1002;</i>刷新</a>
          <div class="spacer-gray"></div>
        ...
        <table class="layui-table" lay-even>
          ...
          <tbody>
          <?php foreach ($products['data'] as $v) { ?>
            <tr>
              <td><input type="checkbox" name="ids[<?php echo $v['id'] ?>]" value="<?php echo $v['id'] ?>" lay-skin="primary" lay-filter="checkOne"></td>
              <td><?php echo $v['id']; ?></td>
              <td>
                <a class="list-title" href="<<?php if($v['url']){ echo $v['url'];}else{ echo url('index/product/show','id='.$v['id']);} ?>" target="_blank"><?php echo $v['title']; ?></a>
                <?php if($v['image_url']){ ?>
              ...
              <td>
                <a href="<?php echo url('product/edit',['id'=>$v['id']]) ?>" class="layui-btn layui-btn-mini" title="编辑"><i class="layui-icon">&#xe642;</i></a>
                <a class="layui-btn layui-btn-danger layui-btn-mini del_btn" product-id="<?php echo $v['id'] ?>" title="删除"><i class="layui-icon">&#xe640;</i></a>
              </td>
            </tr>
          <?php } ?>
          </tbody>
  ...
  $('#batches_delete').click(function(){
    ...
    layer.confirm('确定删除['+ids+']?', function(index){
      ...
      $.ajax({
        type:'post',
        dataType:'json',
        url:'{:url("product/batches_delete")}',
        data:{'ids':ids},
        success:function(data){
          if(data.code == 200){
            show_msg(data.msg,1,'reload');
